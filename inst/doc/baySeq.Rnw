...
\title{baySeq: Empirical Bayesian analysis of patterns of differential expression in count data (v.1.1.3)}
...
Having acquired a set of prior distributions on the rate parameter of the Poisson distribution, we can calculate the posterior likelihoods of each model for each tag. We need to pass an initial prior likelihood on each model; the \verb'prs' parameter. If \verb'estimatePriors = TRUE' then the prior likelihood on each model will be iteratively updated.
...
long, this would be much poorer evidence of differential expression. We can use the 'segData' class to explore such data.
...
As before, the data are simulated such that the first hundred segments show differential expression between the first five libraries and the second five libraries. We thus establish two groups.
...
We now combine the count data, library sizes and user-defined groups, together with the segment lengths, into a \verb'segData' object.

<<>>=
SD <- new("segData", data = simSeg[,-1], libsizes = libsizes, groups = groups, seglens = simSeg[,1])
@

We can also optionally add annotation details into the \verb'@annotation' slot of the \verb'segData' object.

<<>>=
SD@annotation <- data.frame(name = paste("gene", 1:1000, sep = "_"))
@ 

At present, for evaluating differential expression between segments, only the Negative Binomial approach is implemented. We calculate the priors as before.

<<>>=
SDP.NBML <- getPriors.NB(SD, samplesize = 1000, estimation = "ML", cl = cl)
@

We can then calculate the posterior likelihoods as before.
<<>>=
SDPost.NBML <- getLikelihoods.NBboot(SDP.NBML, prs = c(0.5,0.5), estimatePriors = TRUE, bootStraps = 2, cl = cl)
SDPost.NBML@estProps
SDPost.NBML@posteriors[1:10,]
SDPost.NBML@posteriors[101:110,]
@

If we ignore segment length, and merely use the count data, we could acquire posteriors as before.

<<>>=
CSD <- new("countData", data = simSeg[,-1], libsizes = libsizes, groups = groups)
CSD@annotation <- data.frame(name = paste("gene", 1:1000, sep = "_"))
CSDP.NBML <- getPriors.NB(CSD, samplesize = 1000, estimation = "ML", cl = cl)
CSDPost.NBML <- getLikelihoods.NBboot(CSDP.NBML, prs = c(0.5,0.5), estimatePriors = TRUE, bootStraps = 2, cl = cl)
@

If we look at the false discovery rates (Figure~\ref{FPseglen}), we find that ignoring segment lengths can have a small negative effect on the results in these data; the analysis of the \verb'segData' object (blue) has a lower false discovery rate than the analysis of the \verb'countData' object (red).

\begin{figure}[!ht]
\begin{center}
<<echo = FALSE, fig = TRUE, label=FPseglen>>=
SD.FPs <- 1:nrow(simSeg) - getTPs(SDPost.NBML, group = 2, TPs = 1:100)
CSD.FPs <- 1:nrow(simSeg) - getTPs(CSDPost.NBML, group = 2, TPs = 1:100)
plot(x = NA, y = NA, xlim = c(0, 100), ylim = c(0, log(1000)), xlab = "Number of counts selected", ylab = "(Log) False Positives")
lines(x = 1:1000, y = log(CSD.FPs[1:1000]), type = "l", col = "red")
lines(x = 1:1000, y = log(SD.FPs[1:1000]), type = "l", col = "blue")
@
\caption{(Log) false positive rates for differentially expressed region discovery in a simulated dataset using the the Negative Binomial approach considering region lengths (blue) and neglecting region lengths (red).}
\label{FPseglen}
\end{center}
\end{figure}

We are also able to ask if any segments have no true expression by setting \verb'nullData = TRUE'. If the distribution of rates at each segment is bimodal, this approach will be able to differentiate between segments which have no true expression of any kind, and those which have expression (and may be differentially expressed). If we use this option, we implicitly introduce another grouping. The \verb'prs' vector, which defines the prior likelihood of each grouping, should thus sum to less than 1, with the residual going to the implicit group. We can use this method with either \verb'countData' or \verb'segData' objects; however, to ignore segment length may have a significant effect on the results.

In the \verb'simSeg' object, the second hundred rows are simulated to show no true expression.

<<>>=
NSDPost.NBML <- getLikelihoods.NBboot(SDP.NBML, prs = c(0.4,0.3), estimatePriors = TRUE, nullData = TRUE, bootStraps = 3, cl = cl)
NCSDPost.NBML <- getLikelihoods.NBboot(CSDP.NBML, prs = c(0.4,0.3), estimatePriors = TRUE, nullData = TRUE, bootStraps = 3, cl = cl)
@

We can see the top segments for which there is no true expression by setting \verb'group = NULL' in the \verb'topCounts' function. Similarly, we can find true positive rates for by setting \verb'group = NULL' in the \verb'getTPs' function. Figure~\ref{fpNull} shows that neglecting the segment lengths causes a substantial difference in false positive rates in the analysis of the \verb'countData' object (blue) compared to the analysis of the \verb'segData' object (red) in the identification of segments for which there is no true expression.
...
lines(x = 1:1000, y = log(NSD.FPs[1:1000]), type = "l", col = "blue")
@
\caption{(Log) false positive rates for discovery of regions with no genuine expression in a simulated dataset using the the Negative Binomial approach considering region lengths (blue) and neglecting region lengths (red).}
\label{fpNull}
\end{center}
\end{figure}

In order to look for segments for which there is no true expression, we assume that the rates of production of tags are bimodally distributed. If this is not the case, then this approach may cause serious problems in the analysis of the data. It is possible to gain some idea of whether the rates are bimodally distributed by examining the priors estimated (by Negative Binomial methods) for a \verb'countData' or \verb'segData' object by using the function \verb'plotPriors' for some group; ideally, the group which defines non-differentially expressed data. Figure~\ref{plotPriors} shows that these data are truly bimodal and thus this is a sensible approach.
