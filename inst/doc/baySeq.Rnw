...
We then acquire posterior likelihoods as before, estimating the proportions of differentially expressed counts.
<<>>=
...
<<>>=
topCounts(CDPost.NBML, group = "DE")  
@ 
...
plotPosteriors(CDPost.NBML, group = "DE", col = c(rep("red", 100), rep("black", 900)))
@ 
...
<<>>=
cDPair@replicates <- as.factor(c("D3/D3", "D3/D3", "WT/D3", "WT/D3"))
@ 
...
<<>>=
cDPair@groups <- list(NDE = NDE, mobile = mobile)  
@ 
