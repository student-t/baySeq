...
\title{baySeq: Empirical Bayesian analysis of patterns of differential expression in count data (v.1.1.6)}
...
<<>>=
CDPost.NBML <- getLikelihoods.NBboot(CDP.NBML, prs = c(0.5,0.5), estimatePriors = TRUE, bootStraps = 3, cl = cl)
...
<<echo = TRUE, fig = TRUE, label=FPsPlot>>=
...
CDfactPost.NBML <- getLikelihoods.NBboot(CDfactP.NBML, prs = c(0.2, 0.3,0.5), estimatePriors = TRUE, bootStraps = 3, cl = cl)
...
We can calculate the priors for this segData object as before, for both the Poisson-Gamma method and the Negative Binomial method.
...
<<>>=
SDPost.Pois <- getLikelihoods.Pois(SDP.Pois, prs = c(0.5,0.5), estimatePriors = TRUE, cl = cl)  
SDPost.NBML <- getLikelihoods.NBboot(SDP.NBML, prs = c(0.5,0.5), estimatePriors = TRUE, bootStraps = 3, cl = cl)
@
...
CSDPost.NBML <- getLikelihoods.NBboot(CSDP.NBML, prs = c(0.5,0.5), estimatePriors = TRUE, bootStraps = 3, cl = cl)
CSDP.Pois <- getPriors.Pois(CSD, samplesize = 20, cl = cl)
CSDPost.Pois <- getLikelihoods.Pois(CSDP.Pois, prs = c(0.5,0.5), estimatePriors = TRUE, cl = cl)
@
